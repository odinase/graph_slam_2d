...
class GraphSLAM:
    ...
    def new_observation(self, z):
        ...
        if numLmks > 0:
            ...
            zass_s = utils.polar2cart(zass)
            
            # We skip the transform between body frame and sensor frame as these cancel in the end (the sensor frame is fixed relative body frame)
            ...
            zass_k = utils.transform(T_kb, zass_s)

            # Polar coordinates, ordering consistent with lmk_idx
            zass_kp = utils.cart2polar(zass_k)

            # Get two lists for ranges and bearings for convenience
            ranges = zass_kp[:,0]
            bearings = [gtsam.Rot2(b) for b in zass_kp[:,1]]
